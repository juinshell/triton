...
SmallVector<unsigned>
LinearEncodingAttr::getShapePerCTATile(ArrayRef<int64_t> tensorShape) const {
  // FIXME Remove this function from the Distributed API
  // It's not entirely clear what this function should return (it doesn't use
  // tensorShape!) We follow the definition in BlockedEncodingAttr, but we
  // should just remove it
  auto ll = getLinearLayout();
  SmallVector<unsigned> shape;
  for (unsigned d = 0, n = getRepOrder().size(); d < n; ++d) {
    shape.push_back(getSizePerThread()[d] * getThreadsPerWarp()[d] *
                    getWarpsPerCTA()[d]);
  }
  return shape;
